...
from apps.utils.exceptions import ValidationError, PermissionDeniedError, ObjectNotFoundError
...
class TestProjectService(TestCase):
    ...
    def test_create_project_success(self):
        ...
        self.mock_validator.validate_name_format.return_value = clean_name
        ...
        # Assert
        self.mock_validator.validate_name_format.assert_called_once_with(raw_name)
        self.mock_dal.filter_by.assert_called_once_with(user=self.user, name__iexact=clean_name)
        queryset_mock.exists.assert_called_once()
    ...
    def test_create_project_duplicate_name(self):
        ...
        self.mock_validator.validate_name_format.return_value = name
    ...
    def test_update_project_permission_denied(self):
        ...
        project_mock.user = Mock(id=999)  # Different user
...
class TestProjectServiceIntegration(TestCase):
    ...
    def test_user_isolation(self):
        ...
        # User B cannot update/delete User A's project
        with self.assertRaises(PermissionDeniedError):
    ...
    def test_name_uniqueness_per_user(self):
        ...
        # User can't create two projects with same name
        self.service.create_project(self.user, 'Same Name')
        with self.assertRaises(ValidationError):
            self.service.create_project(self.user, 'Same Name')
        
        # But different users can have projects with same name
        self.service.create_project(self.other_user, 'Same Name')  # Should work
